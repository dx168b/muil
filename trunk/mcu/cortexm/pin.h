...
struct Pin
{
	...
	// IRQn for NVIC_SetPriority, NVIC_EnableIRQ etc
	static const IRQn_Type ext_irqn =
		(pin_no == 0)                    ? EXTI0_IRQn :
		(pin_no == 1)                    ? EXTI1_IRQn :
		(pin_no == 2)                    ? EXTI2_IRQn :
		(pin_no == 3)                    ? EXTI3_IRQn :
		(pin_no == 4)                    ? EXTI4_IRQn :
		(pin_no >= 5) && (pin_no <= 9)   ? EXTI9_5_IRQn :
		(pin_no >= 10) && (pin_no <= 15) ? EXTI15_10_IRQn :
		IRQn(0);
	...
	static void AssignPinExtInt()
	{
		static const uint32_t shft = (pin_no % 4) * 4;
		static const uint32_t EXTICRi = pin_no / 4;
		AFIO->EXTICR[EXTICRi] = (AFIO->EXTICR[EXTICRi] & ~(0xF << shft)) | (port_no << shft);
	}

	static void EnablePinExtInt(bool raise, bool fall)
	{
		EXTI->IMR = (EXTI->IMR & ~mask) | (raise || fall ? mask : 0);
		EXTI->RTSR = (EXTI->RTSR & ~mask) | (raise ? mask : 0);
		EXTI->FTSR = (EXTI->FTSR & ~mask) | (fall ? mask : 0);
	}
};
